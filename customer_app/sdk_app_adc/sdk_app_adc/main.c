...
#define normal_adc_mode   1

void adc_data_callback(int mode, uint32_t *ptr, uint32_t data_size)
{
    ...
    if (mode == 0) {
        data = hal_prase_adc_data(ptr, 4, 1);
        printf("data = %ld \r\n", data);
    } else {
        hal_parse_data_arr(ptr, output, data_size);
        for (i = 0; i < 10; i++) {
            printf("%ld ", output[i]);
        }
    }

    printf("time = %lld \r\n", (bl_timer_now_us64() - count_time));
    count_time = bl_timer_now_us64();
    
}
static void aos_test_proc(void *pvParameters)
{
    int32_t data;
    int i;
    int ret = 0;
...
#if normal_adc_mode
    ret = hal_adc_init(0, 1300, 0, 4);
#else
    ret = hal_adc_init(1, 16000, 100, 4);
#endif
...
#if normal_adc_mode 
    hal_adc_add_channel(5);
    hal_adc_add_channel(6);
    hal_adc_add_channel(9);
    hal_adc_add_channel(10);
    hal_adc_add_channel(11);
    hal_adc_add_channel(12);
    hal_adc_add_channel(13);
#endif
...
#if normal_adc_mode
        for (i = 0; i < 16; i++) {
            data = hal_adc_get_data(i, 1);
            if (data < 0) {
                //printf("gpio channel not init or gpio not correct \r\n");
            } else {
                printf("gpio = %d data = %ld mv \r\n", i, data);
            }
        }
#endif
    }
...
void bfl_main()
{
    ...
    static StackType_t aos_test_proc_stack[1024];
    static StaticTask_t aos_test_proc_task;
    ...
    xTaskCreateStatic(aos_test_proc, (char*)"event_test", 1024, NULL, 15, aos_test_proc_stack, &aos_test_proc_task);
