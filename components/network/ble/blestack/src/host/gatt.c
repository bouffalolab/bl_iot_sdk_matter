...
#if defined(BFLB_BLE)
    struct ccc_save_struct save_tmp;  
#else
...
#if defined(BFLB_BLE)
    memcpy(&save_tmp.addr, save.addr, sizeof(bt_addr_le_t));
    save_tmp.count = save.count;
    memcpy(save_tmp.store, save.store, sizeof(save.store));
    err = bt_settings_set_bin(NV_GATT_CCC, (u8_t *)&save_tmp, sizeof(save_tmp));
...
void bt_gatt_clear_ccc(u8_t id, const bt_addr_le_t *addr)
{
#if defined(BFLB_BLE)
    struct ccc_save_struct ccc_array[CONFIG_BT_MAX_CONN];
    int index;

    memset(&ccc_array[0], 0 , sizeof(ccc_array));
    
    if(bt_settings_get_bin(NV_GATT_CCC, (u8_t *)ccc_array, sizeof(ccc_array)))
        return;
    
    for(index = 0; index < CONFIG_BT_MAX_CONN; index++) {
        if (!bt_addr_le_cmp(&ccc_array[index].addr, addr)){
            break;
        }
    }
    ...
    memset(ccc_array+index, 0 , sizeof(struct ccc_save_struct));
    bt_settings_set_bin(NV_GATT_CCC, (u8_t *)ccc_array, sizeof(ccc_array));
...
#if defined(BFLB_BLE)
static int ccc_set(struct ccc_save_struct *ccc_save)
{
    ...
    memcpy(ccc_store, ccc_save->store, len);
    memcpy(&load.addr, &ccc_save->addr, sizeof(bt_addr_le_t));
    load.entry = ccc_store;
    load.count = ccc_save->count;
...
void bt_gatt_ccc_load()
{
    ...
    for(index = 0; index < ARRAY_SIZE(ccc_array); index++){
        ccc_set(ccc_array + index);
    }     
}
#else
