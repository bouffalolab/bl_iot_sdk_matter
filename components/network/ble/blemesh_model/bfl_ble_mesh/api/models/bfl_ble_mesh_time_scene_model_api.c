...
bfl_err_t bfl_ble_mesh_register_time_scene_client_callback(bfl_ble_mesh_time_scene_client_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_TIME_SCENE_CLIENT, callback) == 0 ? BFL_OK : BFL_FAIL);
}
        ...
        bfl_ble_mesh_time_scene_client_get_state_t *get_state)
{
    ...
    if (params == NULL || params->model == NULL ||
        ...
        (params->opcode == BFL_BLE_MESH_MODEL_OP_SCHEDULER_ACT_GET && get_state == NULL)) {
...
bfl_err_t bfl_ble_mesh_register_time_scene_server_callback(bfl_ble_mesh_time_scene_server_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_TIME_SCENE_SERVER, callback) == 0 ? BFL_OK : BFL_FAIL);
}
