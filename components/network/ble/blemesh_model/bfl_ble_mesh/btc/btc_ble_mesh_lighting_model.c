...
static inline void btc_ble_mesh_lighting_client_cb_to_app(bfl_ble_mesh_light_client_cb_event_t event,
        bfl_ble_mesh_light_client_cb_param_t *param)
{
    bfl_ble_mesh_light_client_cb_t btc_ble_mesh_cb =
        (bfl_ble_mesh_light_client_cb_t)btc_profile_cb_get(BTC_PID_LIGHTING_CLIENT);
    if (btc_ble_mesh_cb) {
...
void btc_ble_mesh_lighting_client_arg_deep_copy(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    case BTC_BLE_MESH_ACT_LIGHTING_CLIENT_GET_STATE: {
        dst->light_client_get_state.params = (bfl_ble_mesh_client_common_param_t *)bt_mesh_malloc(sizeof(bfl_ble_mesh_client_common_param_t));
        ...
        if (src->light_client_get_state.get_state) {
            dst->light_client_get_state.get_state = (bfl_ble_mesh_light_client_get_state_t *)bt_mesh_malloc(sizeof(bfl_ble_mesh_light_client_get_state_t));
            if (dst->light_client_get_state.get_state) {
                memcpy(dst->light_client_get_state.get_state, src->light_client_get_state.get_state,
                    sizeof(bfl_ble_mesh_light_client_get_state_t));
            } else {
                BT_ERR("%s, Failed to allocate memory, act %d", __func__, msg->act);
            }
        }
    ...
    case BTC_BLE_MESH_ACT_LIGHTING_CLIENT_SET_STATE: {
        dst->light_client_set_state.params = (bfl_ble_mesh_client_common_param_t *)bt_mesh_malloc(sizeof(bfl_ble_mesh_client_common_param_t));
        dst->light_client_set_state.set_state = (bfl_ble_mesh_light_client_set_state_t *)bt_mesh_malloc(sizeof(bfl_ble_mesh_light_client_set_state_t));
        if (dst->light_client_set_state.params && dst->light_client_set_state.set_state) {
            ...
            memcpy(dst->light_client_set_state.set_state, src->light_client_set_state.set_state,
                   sizeof(bfl_ble_mesh_light_client_set_state_t));
        } else {
            BT_ERR("%s, Failed to allocate memory, act %d", __func__, msg->act);
        }
        break;
...
{
    bfl_ble_mesh_light_client_cb_param_t *p_dest_data = (bfl_ble_mesh_light_client_cb_param_t *)p_dest;
    bfl_ble_mesh_light_client_cb_param_t *p_src_data = (bfl_ble_mesh_light_client_cb_param_t *)p_src;
    u16_t length = 0U;
    ...
    case BFL_BLE_MESH_LIGHT_CLIENT_PUBLISH_EVT:
        if (p_src_data->params) {
            ...
            case BFL_BLE_MESH_MODEL_OP_LIGHT_LC_PROPERTY_GET:
            case BFL_BLE_MESH_MODEL_OP_LIGHT_LC_PROPERTY_SET:
            case BFL_BLE_MESH_MODEL_OP_LIGHT_LC_PROPERTY_STATUS:
...
{
    bfl_ble_mesh_light_client_cb_param_t *arg = NULL;
    ...
    arg = (bfl_ble_mesh_light_client_cb_param_t *)(msg->arg);
...
static void btc_ble_mesh_lighting_client_callback(bfl_ble_mesh_light_client_cb_param_t *cb_params, uint8_t act)
{
    btc_msg_t msg = {0};
    ...
    btc_transfer_context(&msg, cb_params,
                         sizeof(bfl_ble_mesh_light_client_cb_param_t), btc_ble_mesh_lighting_client_copy_req_data);
}
...
{
    bfl_ble_mesh_light_client_cb_param_t cb_params = {0};
    ...
    case BTC_BLE_MESH_EVT_LIGHTING_CLIENT_GET_STATE:
        act = BFL_BLE_MESH_LIGHT_CLIENT_GET_STATE_EVT;
        break;
    case BTC_BLE_MESH_EVT_LIGHTING_CLIENT_SET_STATE:
        act = BFL_BLE_MESH_LIGHT_CLIENT_SET_STATE_EVT;
        break;
    case BTC_BLE_MESH_EVT_LIGHTING_CLIENT_PUBLISH:
        act = BFL_BLE_MESH_LIGHT_CLIENT_PUBLISH_EVT;
        break;
    case BTC_BLE_MESH_EVT_LIGHTING_CLIENT_TIMEOUT:
        act = BFL_BLE_MESH_LIGHT_CLIENT_TIMEOUT_EVT;
        break;
    default:
...
void btc_ble_mesh_lighting_client_call_handler(btc_msg_t *msg)
{
    ...
    btc_ble_mesh_lighting_client_args_t *arg = NULL;
    bfl_ble_mesh_light_client_cb_param_t cb = {0};
    ...
    case BTC_BLE_MESH_ACT_LIGHTING_CLIENT_GET_STATE: {
        ...
        if (cb.error_code) {
            ...
            btc_ble_mesh_lighting_client_callback(&cb, BFL_BLE_MESH_LIGHT_CLIENT_GET_STATE_EVT);
        }
        break;
    }
    case BTC_BLE_MESH_ACT_LIGHTING_CLIENT_SET_STATE: {
        ...
        if (cb.error_code) {
            /* If send failed, callback error_code to app layer immediately */
            btc_ble_mesh_lighting_client_callback(&cb, BFL_BLE_MESH_LIGHT_CLIENT_SET_STATE_EVT);
        }
        break;
    }
    default:
...
{
    bfl_ble_mesh_light_client_cb_param_t *param = NULL;
    ...
    param = (bfl_ble_mesh_light_client_cb_param_t *)(msg->arg);

    if (msg->act < BFL_BLE_MESH_LIGHT_CLIENT_EVT_MAX) {
...
{
    bfl_ble_mesh_lighting_server_cb_t btc_ble_mesh_cb =
        (bfl_ble_mesh_lighting_server_cb_t)btc_profile_cb_get(BTC_PID_LIGHTING_SERVER);
    if (btc_ble_mesh_cb) {
        btc_ble_mesh_cb(event, param);
    }
}
...
{
    bfl_ble_mesh_lighting_server_cb_param_t *p_dest_data = (bfl_ble_mesh_lighting_server_cb_param_t *)p_dest;
    bfl_ble_mesh_lighting_server_cb_param_t *p_src_data = (bfl_ble_mesh_lighting_server_cb_param_t *)p_src;
    u16_t length = 0U;
    ...
    case BFL_BLE_MESH_LIGHTING_SERVER_STATE_CHANGE_EVT:
        if (p_src_data->ctx.recv_op == BFL_BLE_MESH_MODEL_OP_LIGHT_LC_PROPERTY_SET ||
                p_src_data->ctx.recv_op == BFL_BLE_MESH_MODEL_OP_LIGHT_LC_PROPERTY_SET_UNACK) {
    ...
    case BFL_BLE_MESH_LIGHTING_SERVER_RECV_SET_MSG_EVT:
        ...
        break;
    case BFL_BLE_MESH_LIGHTING_SERVER_RECV_STATUS_MSG_EVT:
        if (p_src_data->ctx.recv_op == BFL_BLE_MESH_MODEL_OP_SENSOR_STATUS) {
...
{
    bfl_ble_mesh_lighting_server_cb_param_t *arg = NULL;
    ...
    arg = (bfl_ble_mesh_lighting_server_cb_param_t *)(msg->arg);

    switch (msg->act) {
    case BFL_BLE_MESH_LIGHTING_SERVER_STATE_CHANGE_EVT:
        if (arg->ctx.recv_op == BFL_BLE_MESH_MODEL_OP_LIGHT_LC_PROPERTY_SET ||
                arg->ctx.recv_op == BFL_BLE_MESH_MODEL_OP_LIGHT_LC_PROPERTY_SET_UNACK) {
    ...
    case BFL_BLE_MESH_LIGHTING_SERVER_RECV_SET_MSG_EVT:
        ...
        break;
    case BFL_BLE_MESH_LIGHTING_SERVER_RECV_STATUS_MSG_EVT:
        if (arg->ctx.recv_op == BFL_BLE_MESH_MODEL_OP_SENSOR_STATUS) {
...
static void btc_ble_mesh_lighting_server_callback(bfl_ble_mesh_lighting_server_cb_param_t *cb_params, uint8_t act)
{
    btc_msg_t msg = {0};
    ...
    btc_transfer_context(
        &msg, cb_params, sizeof(bfl_ble_mesh_lighting_server_cb_param_t), btc_ble_mesh_lighting_server_copy_req_data);
}
...
void bt_mesh_lighting_server_cb_evt_to_btc(u8_t evt_type,
        ...
        const u8_t *val, size_t len)
{
    bfl_ble_mesh_lighting_server_cb_param_t cb_params = {0};
    size_t length = 0U;
    uint8_t act = 0U;
    ...
    case BTC_BLE_MESH_EVT_LIGHTING_SERVER_STATE_CHANGE:
        act = BFL_BLE_MESH_LIGHTING_SERVER_STATE_CHANGE_EVT;
        break;
    case BTC_BLE_MESH_EVT_LIGHTING_SERVER_RECV_GET_MSG:
        act = BFL_BLE_MESH_LIGHTING_SERVER_RECV_GET_MSG_EVT;
        break;
    case BTC_BLE_MESH_EVT_LIGHTING_SERVER_RECV_SET_MSG:
        act = BFL_BLE_MESH_LIGHTING_SERVER_RECV_SET_MSG_EVT;
        break;
    case BTC_BLE_MESH_EVT_LIGHTING_SERVER_RECV_STATUS_MSG:
        act = BFL_BLE_MESH_LIGHTING_SERVER_RECV_STATUS_MSG_EVT;
        break;
    default:
...
{
    bfl_ble_mesh_lighting_server_cb_param_t *param = NULL;
    ...
    param = (bfl_ble_mesh_lighting_server_cb_param_t *)(msg->arg);

    if (msg->act < BFL_BLE_MESH_LIGHTING_SERVER_EVT_MAX) {
