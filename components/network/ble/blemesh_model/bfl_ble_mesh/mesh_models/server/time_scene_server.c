                             ...
                             bool publish, u16_t opcode)
{
    ...
    case BLE_MESH_MODEL_OP_TIME_STATUS:
        if (model->id == BT_MESH_MODEL_ID_TIME_SRV) {
        ...
        } else if (model->id == BT_MESH_MODEL_ID_TIME_SETUP_SRV) {
                                       ...
                                       u8_t status_code, bool publish)
{
    ...
    if (publish == false) {
        msg = bt_mesh_alloc_buf(MIN(BT_MESH_TX_SDU_MAX, BLE_MESH_SERVER_RSP_MAX_LEN));
        if (msg == NULL) {
            BT_ERR("%s, Failed to allocate memory", __func__);
            return;
        }
                         ...
                         struct net_buf_simple *buf)
{
    ...
    case BLE_MESH_MODEL_OP_SCENE_DELETE_UNACK: {
        ...
        if (srv->state->target_scene == scene_number &&
            ...
            scene_model = bt_mesh_model_find(bt_mesh_model_elem(model), BT_MESH_MODEL_ID_SCENE_SRV);
            if (scene_model == NULL) {
...
int bt_mesh_time_srv_init(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
    ...
    if (bt_mesh_model_find(element, BT_MESH_MODEL_ID_TIME_SETUP_SRV) == NULL) {
        BT_WARN("%s, Time Setup Server is not present", __func__);
        /* Just give a warning here, continue with the initialization */
    }

    model->op = time_srv_op;
    return time_scene_server_init(model);
}

int bt_mesh_time_setup_srv_init(struct bt_mesh_model *model)
{
    ...
    model->op = time_setup_srv_op;
    return time_scene_server_init(model);
}

int bt_mesh_scene_srv_init(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
    ...
    model->op = scene_srv_op;
    return time_scene_server_init(model);
}

int bt_mesh_scene_setup_srv_init(struct bt_mesh_model *model)
{
    /* The model may be present only on the Primary element of a node. */
    //if (primary == false) {
    ...
    //    /* Just give a warning here, continue with the initialization */
    //}
    model->op = scene_setup_srv_op;
    return time_scene_server_init(model);
}

int bt_mesh_scheduler_srv_init(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
     ...
     */
    struct bt_mesh_elem *element = bt_mesh_model_elem(model);
    if (bt_mesh_model_find(element, BT_MESH_MODEL_ID_SCHEDULER_SETUP_SRV) == NULL) {
        BT_WARN("%s, Scheduler Setup Server is not present", __func__);
        /* Just give a warning here, continue with the initialization */
    }
    if (bt_mesh_model_find(element, BT_MESH_MODEL_ID_TIME_SRV) == NULL) {
        BT_WARN("%s, Time Server is not present", __func__);
        /* Just give a warning here, continue with the initialization */
    }
    model->op = scheduler_srv_op;
    return time_scene_server_init(model);
}

int bt_mesh_scheduler_setup_srv_init(struct bt_mesh_model *model)
{
    /* The model may be present only on the Primary element of a node. */
    //if (primary == false) {
    ...
    //    /* Just give a warning here, continue with the initialization */
    //}
    model->op = scheduler_setup_srv_op;
    return time_scene_server_init(model);
}
...
int bt_mesh_time_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Time Server has no publication support", __func__);
        return -EINVAL;
...
int bt_mesh_time_setup_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub) {
        BT_ERR("%s, Time Setup Server shall not support publication", __func__);
        return -EINVAL;
...
int bt_mesh_scene_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Scene Server has no publication support", __func__);
        return -EINVAL;
...
int bt_mesh_scene_setup_srv_deinit(struct bt_mesh_model *model)
{
    return time_scene_server_deinit(model);
}

int bt_mesh_scheduler_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Scheduler Server has no publication support", __func__);
        return -EINVAL;
...
int bt_mesh_scheduler_setup_srv_deinit(struct bt_mesh_model *model)
{
    return time_scene_server_deinit(model);
}
