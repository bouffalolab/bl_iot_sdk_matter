                               ...
                               struct net_buf_simple *buf)
{
    ...
    sensor_model = bt_mesh_model_find(element, BT_MESH_MODEL_ID_SENSOR_SRV);
...
static void update_sensor_periodic_pub(struct bt_mesh_model *model, u16_t prop_id)
{
    ...
    if (model->id != BT_MESH_MODEL_ID_SENSOR_SRV) {
...
int bt_mesh_sensor_srv_init(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
    ...
    if (bt_mesh_model_find(element, BT_MESH_MODEL_ID_SENSOR_SETUP_SRV) == NULL) {
        BT_WARN("%s, Sensor Setup Server is not present", __func__);
        /* Just give a warning here, continue with the initialization */
    }
    model->op = sensor_srv_op;
    return sensor_server_init(model);
}

int bt_mesh_sensor_setup_srv_init(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
...
int bt_mesh_sensor_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Sensor Server has no publication support", __func__);
        return -EINVAL;
...
int bt_mesh_sensor_setup_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Sensor Setup Server has no publication support", __func__);
        return -EINVAL;
...
const struct bt_mesh_model_cb bt_mesh_sensor_srv_cb = {
	.init = bt_mesh_sensor_srv_init,
};

const struct bt_mesh_model_cb bt_mesh_sensor_setup_srv_cb = {
	.init = bt_mesh_sensor_setup_srv_init,
};
