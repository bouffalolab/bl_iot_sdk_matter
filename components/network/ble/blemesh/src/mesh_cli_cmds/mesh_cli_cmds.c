...
static void cdb_print_nodes(void)
{
	...
	vOutputString("Address  Elements  Flags  %-32s  DevKey", "UUID\n");
...
static void blemesh_beacon_listen(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	if (val) {
		...
		prov.node_added = print_node_added;
	} else {
		prov.unprovisioned_beacon = NULL;
		prov.node_added = NULL;
	}
	return;
}
...
static void blemesh_net_key_get(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	vOutputString("NetKeys known by 0x%04x:", net.dst);
	for (i = 0; i < cnt; i++) {
...
static void blemesh_net_key_del(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	get_uint16_from_string(&argv[1], &key_net_idx);
...
static void blemesh_app_key_add(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	get_uint16_from_string(&argv[1], &key_net_idx);
	get_uint16_from_string(&argv[2], &key_app_idx);
			    ...
			    char *argv[])
{
	...
	get_uint16_from_string(&argv[1], &elem_addr);
	get_uint16_from_string(&argv[2], &mod_app_idx);
	get_uint16_from_string(&argv[3], &mod_id);
			      ...
			      char *argv[])
{
	...
	if (argc > 3) {
		get_uint16_from_string(&argv[3], &cid);
...
static void blemesh_mod_sub_add(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	get_uint16_from_string(&argv[1], &elem_addr);
	get_uint16_from_string(&argv[2], &sub_addr);
	get_uint16_from_string(&argv[3], &mod_id);
...
static void blemesh_mod_sub_ow_va(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_va_overwrite(net.net_idx, net.dst, elem_addr,
				     label,
				     mod_id, &virt_addr,
				     &status);
	}
		       ...
		       u16_t cid, int argc, char *argv[])
{
	...
	get_uint16_from_string(&argv[0], &pub.addr);
	get_uint16_from_string(&argv[1], &pub.app_idx);
	get_uint8_from_string(&argv[2], (uint8_t*)&pub.cred_flag);
	get_uint8_from_string(&argv[3], &pub.ttl);
	get_uint8_from_string(&argv[4], &pub.period);

	get_uint8_from_string(&argv[5], &count);
	if (count > 7) {
		vOutputString("Invalid retransmit count\n");
		return;
	}

	get_uint16_from_string(&argv[6], &interval);
			  ...
			  u16_t cid, int argc, char *argv[])
{
	...
	get_bytearray_from_string(&argv[0], label, sizeof(label));
	get_uint16_from_string(&argv[1], &pub.app_idx);
	get_uint8_from_string(&argv[2], (uint8_t*)&pub.cred_flag);
	get_uint8_from_string(&argv[3], &pub.ttl);
	get_uint8_from_string(&argv[4], &pub.period);
...
static void hb_sub_set(size_t argc, char *argv[])
{
	...
	get_uint16_from_string(&argv[1], &sub.src);
	get_uint16_from_string(&argv[2], &sub.dst);
	get_uint8_from_string(&argv[3], &sub.period);
...
static void hb_pub_set(size_t argc, char *argv[])
{
	...
	get_uint16_from_string(&argv[1], &pub.dst);
	get_uint8_from_string(&argv[2], &pub.count);
	get_uint8_from_string(&argv[3], &pub.period);
	get_uint8_from_string(&argv[4], &pub.ttl);
	get_uint16_from_string(&argv[5], &pub.feat);
	get_uint16_from_string(&argv[6], &pub.net_idx);
...
static void blemesh_krp_get(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	get_uint16_from_string(&argv[1], &krp_buf.NetKeyIndex);

	err = bt_mesh_cfg_krp_get(net.net_idx, net.dst,
					&status, &krp_buf);
	if (err) {
...
static void blemesh_krp_set(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	get_uint16_from_string(&argv[1], &krp_buf.NetKeyIndex);
	get_uint8_from_string(&argv[2], &krp_buf.Phase);
	err = bt_mesh_cfg_krp_set(net.net_idx, net.dst,
					&status, &krp_buf);
	if (err) {
		vOutputString("Getting Key Refresh Phase failed (err %d)\n", err);
		return;
...
static void blemesh_node_identify(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	if (argc == 3) {
		get_uint16_from_string(&argv[1], &netkeyindex);
		get_uint8_from_string(&argv[2], &identity);
...
static void blemesh_node_reset(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	if (argc == 1) {
		err = bt_mesh_cfg_node_reset(net.net_idx, net.dst);
	}
...
static void blemesh_network_trans(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	if (argc == 3) {
		get_uint8_from_string(&argv[1], &count);
		get_uint8_from_string(&argv[2], &interval);
		err = bt_mesh_cfg_network_transmit_set(net.net_idx, net.dst,
						&count, &interval);
	} else if (argc == 1) {
		err = bt_mesh_cfg_network_transmit_get(net.net_idx, net.dst,
						&count, &interval);
	}
...
static void blemesh_lpn_timeout_get(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	get_uint16_from_string(&argv[1], &lpn_addr);
	err = bt_mesh_cfg_lpn_poll_timeout_get(net.net_idx, net.dst,
...
static void blemesh_clhm_fault(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	if(0 == strcmp(argv[1], "test")){
		get_uint16_from_string(&argv[2], &app_idx);
		get_uint16_from_string(&argv[3], &cid);
		get_uint8_from_string(&argv[4], &test_id);
		if(argc == 5)
			/* Health Fault Test */
			err = bt_mesh_health_fault_test(net.dst, app_idx, cid,
		...
		get_uint16_from_string(&argv[2], &app_idx);
		get_uint16_from_string(&argv[3], &cid);
		err = bt_mesh_health_fault_get(net.dst, app_idx, cid,
						 &test_id, faults, &fault_count);
	}
		...
		if(argc == 4)
			/* Health Fault Clear */
			err = bt_mesh_health_fault_clear(net.dst, app_idx, cid,
							 &test_id, faults, &fault_count);
		else
...
static void blemesh_clhm_period(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	if(0 == strcmp(argv[1], "get")){
		get_uint16_from_string(&argv[2], &app_idx);
		err = bt_mesh_health_period_get(net.dst, app_idx, &divisor);
	}
	else if(0 == strcmp(argv[1], "set")){
		get_uint16_from_string(&argv[2], &app_idx);
		get_uint8_from_string(&argv[3], &divisor);
		if(argc == 4)
			/* Acknowledged*/
			err = bt_mesh_health_period_set(net.dst, app_idx, divisor,
				 &updated_divisor);
		else
			/* Unacknowledged */
			err = bt_mesh_health_period_set(net.dst, app_idx, divisor,
...
static void blemesh_clhm_ats(char *pcWriteBuffer, int xWriteBufferLen, int argc, char *argv[])
{
	...
	if(0 == strcmp(argv[1], "get")){
		get_uint16_from_string(&argv[2], &app_idx);
		err = bt_mesh_health_attention_get(net.dst, app_idx, &attention);
	}
	else if(0 == strcmp(argv[1], "set")){
		get_uint16_from_string(&argv[2], &app_idx);
		get_uint8_from_string(&argv[3], &attention);
		if(argc == 4)
			/* Acknowledged*/
			err = bt_mesh_health_attention_set(net.dst, app_idx, attention,
					 &updated_attention);
		else
			/* Unacknowledged */
			err = bt_mesh_health_attention_set(net.dst, app_idx, attention,
