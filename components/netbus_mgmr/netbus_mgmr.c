...
free_ap_wrx:
        ...
        case NETBUS_WIFI_MGMR_MSG_TYPE_WIFI_FRAME_SNIFFER_TO_WIFI_TX:
        {
            ...
            log_debug("TX RAW Frame len %u, first 4 bytes:\r\n", msg.u.cmd.data_len);
...
static void init_tx_desc(netbus_wifi_mgmr_ctx_t *env)
{
    ...
    for (i = 0; i < TX_PBUF_DESC_PBUF_MAX_NUM; ++i) {
        ...
        if (p) {
            log_debug("preallocated pbuf %p, p->next %p\r\n", p, p->next);
            desc->pbufs[i] = p;
            // backup
...
#if 0
    log_debug("Preallocated 1514 pbufs num: %d\r\n", alloced_num);
    log_debug("Freeing 2 for local use.\r\n");
