...
void sdio_pbuf_cfm_cb(int idx, void *arg)
{
    /* printf("pbuf_cfm free %d %p\r\n", idx, arg); */
void bl60x_firmwre_mpdu_free_from_isr(void *swdesc_ptr);
    bl60x_firmwre_mpdu_free_from_isr(arg);
}

int tcpip_stack_input(void *swdesc, uint8_t status, uint8_t is_amsdu, void *hwhdr, unsigned int msdu_offset, struct wifi_pkt *pkt)
{
#ifdef GLOBAL_NETBUS_WIFI_APP_ENABLE
