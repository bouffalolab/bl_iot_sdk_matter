...
int wifi_mgmr_sta_connect(wifi_interface_t *wifi_interface, char *ssid, char *psk, wifi_mgmr_network_config_adv_t *adv_conf)
{
    struct wifi_mgmr_api_mac_config wifi_mgmr_api_mac_info;
    int ssid_len = wifiMgmr.wifi_mgmr_stat_info.ssid_len;
    int psk_len  = wifiMgmr.wifi_mgmr_stat_info.psk_len;
    char pmk[65] = {0};
    int ret = 0;

    pmk[64] = 0;

    if (!strncmp(ssid, wifiMgmr.wifi_mgmr_stat_info.ssid, ssid_len) && (strlen(ssid) == ssid_len)
        && !strncmp(psk, wifiMgmr.wifi_mgmr_stat_info.psk, psk_len) && (strlen(psk) == psk_len)){
        printf("connect info same as before\r\n");
    } else {
        printf("connect info changed, need reproduce pmk\r\n");
        wifi_mgmr_psk_cal(psk, ssid, strlen(ssid), pmk);
        wifi_mgmr_sta_pmk_set(pmk);
        printf("new pmk: %s\r\n", pmk);
    }
    wifi_mgmr_sta_ssid_set(ssid);
    wifi_mgmr_sta_psk_set(psk);

    memset(&wifi_mgmr_api_mac_info, 0, sizeof(struct wifi_mgmr_api_mac_config));

    if (adv_conf) {
        wifi_mgmr_api_mac_info.wifi_mgmr_ip_info.ip = adv_conf->ip;
        wifi_mgmr_api_mac_info.wifi_mgmr_ip_info.mask = adv_conf->mask;
        wifi_mgmr_api_mac_info.wifi_mgmr_ip_info.gw = adv_conf->gw;
        wifi_mgmr_api_mac_info.wifi_mgmr_ip_info.dns1 = adv_conf->dns1;
        wifi_mgmr_api_mac_info.wifi_mgmr_ip_info.dns2 = adv_conf->dns2;
        wifi_mgmr_api_mac_info.ip_lease_time = adv_conf->ip_lease_time;
        wifi_mgmr_api_mac_info.band = adv_conf->band;
        wifi_mgmr_api_mac_info.freq = adv_conf->freq;
        if (pmk[0] != 0) {
            memcpy(adv_conf->pmk, pmk, 64);
            pmk[64] = 0;
            printf("update pmk: %s\r\n", pmk);
        }
        if (adv_conf->gw_mac[0] != 0){
            memcpy(wifi_mgmr_api_mac_info.wifi_mgmr_ip_info.gw_mac, adv_conf->gw_mac, 6);
        }
        wifi_mgmr_api_mac_info.dhcp_use = adv_conf->dhcp_use;
        ret = wifi_mgmr_api_connect(ssid, psk, adv_conf->pmk, adv_conf->mac, &wifi_mgmr_api_mac_info);
    } else {
        wifi_mgmr_api_mac_info.dhcp_use = 1;
        ret = wifi_mgmr_api_connect(ssid, psk, NULL, NULL, &wifi_mgmr_api_mac_info);
    }
    return ret;
}
...
void wifi_mgmr_sta_connect_ind_stat_get(wifi_mgmr_sta_connect_ind_stat_info_t *wifi_mgmr_ind_stat)
{
    int ssid_len = wifiMgmr.wifi_mgmr_stat_info.ssid_len;
    int psk_len = wifiMgmr.wifi_mgmr_stat_info.psk_len;
    int pmk_len = wifiMgmr.wifi_mgmr_stat_info.pmk_len;
    if (ssid_len > 0) {
    ...
    if (pmk_len > 0) {
        memcpy(wifi_mgmr_ind_stat->pmk, wifiMgmr.wifi_mgmr_stat_info.pmk, pmk_len);
        wifi_mgmr_ind_stat->pmk[pmk_len] = '\0';
    }
    ...
    printf("ssid: %s, psk: %s, pmk: %s, band: %d, freq: %d, type_ind: %d\r\n",
            wifi_mgmr_ind_stat->ssid,
            wifi_mgmr_ind_stat->psk,
            wifi_mgmr_ind_stat->pmk,
...
void wifi_mgmr_sta_connect_get_network_config(wifi_mgmr_network_config_adv_t *adv_conf)
{
    wifi_mgmr_profile_msg_t profile;
    wifi_mgmr_profile_get(&wifiMgmr, &profile);
    printf("get profile from mgmr IP:%u.%u.%u.%u, "
            "MASK: %u.%u.%u.%u, "
            "Gateway: %u.%u.%u.%u, "
            "dns1: %u.%u.%u.%u, "
            "dns2: %u.%u.%u.%u\r\n",
        (unsigned int)((profile.net_conf.ip & 0x000000FF) >> 0),
        (unsigned int)((profile.net_conf.ip & 0x0000FF00) >> 8),
        (unsigned int)((profile.net_conf.ip & 0x00FF0000) >> 16),
        (unsigned int)((profile.net_conf.ip & 0xFF000000) >> 24),
        (unsigned int)((profile.net_conf.mask & 0x000000FF) >> 0),
        (unsigned int)((profile.net_conf.mask & 0x0000FF00) >> 8),
        (unsigned int)((profile.net_conf.mask & 0x00FF0000) >> 16),
        (unsigned int)((profile.net_conf.mask & 0xFF000000) >> 24),
        (unsigned int)((profile.net_conf.gw & 0x000000FF) >> 0),
        (unsigned int)((profile.net_conf.gw & 0x0000FF00) >> 8),
        (unsigned int)((profile.net_conf.gw & 0x00FF0000) >> 16),
        (unsigned int)((profile.net_conf.gw & 0xFF000000) >> 24),
        (unsigned int)((profile.net_conf.dns1 & 0x000000FF) >> 0),
        (unsigned int)((profile.net_conf.dns1 & 0x0000FF00) >> 8),
        (unsigned int)((profile.net_conf.dns1 & 0x00FF0000) >> 16),
        (unsigned int)((profile.net_conf.dns1 & 0xFF000000) >> 24),
        (unsigned int)((profile.net_conf.dns2 & 0x000000FF) >> 0),
        (unsigned int)((profile.net_conf.dns2 & 0x0000FF00) >> 8),
        (unsigned int)((profile.net_conf.dns2 & 0x00FF0000) >> 16),
        (unsigned int)((profile.net_conf.dns2 & 0xFF000000) >> 24)
    );
    adv_conf->ip = profile.net_conf.ip;
    adv_conf->mask = profile.net_conf.mask;
    adv_conf->gw = profile.net_conf.gw;
    adv_conf->dns1 = profile.net_conf.dns1;
    adv_conf->dns2 = profile.net_conf.dns2;
    adv_conf->timestamp = profile.timestamp;
    adv_conf->ip_lease_time = profile.ip_lease_time;
}

void wifi_mgmr_sta_gw_mac_get(uint8_t *gw_mac)
{
    printf("try to get gw mac\r\n");
    if (wifiMgmr.wlan_sta.netif.arp_gw_ethaddr[0] != 0) {
        printf("gw mac: %02x:%02x:%02x:%02x:%02x:%02x\r\n",
            wifiMgmr.wlan_sta.netif.arp_gw_ethaddr[0],
            wifiMgmr.wlan_sta.netif.arp_gw_ethaddr[1],
            wifiMgmr.wlan_sta.netif.arp_gw_ethaddr[2],
            wifiMgmr.wlan_sta.netif.arp_gw_ethaddr[3],
            wifiMgmr.wlan_sta.netif.arp_gw_ethaddr[4],
            wifiMgmr.wlan_sta.netif.arp_gw_ethaddr[5]
        );
        memcpy(gw_mac, wifiMgmr.wlan_sta.netif.arp_gw_ethaddr, 6);
    } else {
        printf("No valid gw mac, reaset addr\r\n");
        memset(gw_mac, 0, 6);
    }
}
...
void wifi_mgmr_sta_psk_set(char *psk)
{
    ...
    if (len > 0) {
        ...
        wifiMgmr.wifi_mgmr_stat_info.psk_len = len;
    }
}

void wifi_mgmr_sta_pmk_set(char *pmk)
{
    uint16_t len = strlen(pmk);
    if (len > 0) {
       len = len > 64? 64 : len;
       memcpy(wifiMgmr.wifi_mgmr_stat_info.pmk, pmk, len);
       //wifiMgmr.wifi_mgmr_stat_info.pmk[len] = '\0';
       wifiMgmr.wifi_mgmr_stat_info.pmk_len = len;
    }
}
...
int wifi_mgmr_auth_to_ext(int auth)
{
    int ret;
    switch (auth) {
    ...
    case WIFI_EVENT_BEACON_IND_AUTH_WPA_ENT:
        ret = WIFI_AUTH_WPA_ENTERPRISE;
        break;
    case WIFI_EVENT_BEACON_IND_AUTH_UNKNOWN:
        ret = WIFI_AUTH_UNKNOWN;
        break;
    default:
        ret = WIFI_AUTH_UNKNOWN;
        break;
...
int wifi_mgmr_cipher_to_ext(int cipher)
{
    int ret;

    switch (cipher) {
    case WIFI_EVENT_BEACON_IND_CIPHER_NONE:
        ret = WIFI_CIPHER_NONE;
        break;
    ...
    case WIFI_EVENT_BEACON_IND_CIPHER_TKIP:
        ret = WIFI_CIPHER_TKIP;
        break;
    case WIFI_EVENT_BEACON_IND_CIPHER_TKIP_AES:
        ret = WIFI_CIPHER_TKIP_AES;
        break;
    default:
        ret = WIFI_CIPHER_NONE;
        break;
