...
int wifi_mgmr_api_connect(char *ssid, char *psk, char *pmk, uint8_t *mac, struct wifi_mgmr_api_mac_config *wifi_mgmr_api_mac_info)
{
    wifi_mgmr_msg_t *msg;
    ...
    if (wifi_mgmr_api_mac_info->freq > 0) {
        //define the channel
        profile->band = wifi_mgmr_api_mac_info->band;
        profile->freq = wifi_mgmr_api_mac_info->freq;
        printf("wifi mgmr band:%d freq: %d\r\n", profile->band, profile->freq);
    }
    if (wifi_mgmr_api_mac_info->wifi_mgmr_ip_info.ip) {
        profile->net_conf.ip = wifi_mgmr_api_mac_info->wifi_mgmr_ip_info.ip;
        profile->net_conf.mask = wifi_mgmr_api_mac_info->wifi_mgmr_ip_info.mask;
        profile->net_conf.gw = wifi_mgmr_api_mac_info->wifi_mgmr_ip_info.gw;
        profile->net_conf.dns1 = wifi_mgmr_api_mac_info->wifi_mgmr_ip_info.dns1;
        profile->net_conf.dns2 = wifi_mgmr_api_mac_info->wifi_mgmr_ip_info.dns2;
        memcpy(profile->net_conf.gw_mac, wifi_mgmr_api_mac_info->wifi_mgmr_ip_info.gw_mac, 6);
        profile->ip_lease_time = wifi_mgmr_api_mac_info->ip_lease_time;
    }
    profile->dhcp_use = wifi_mgmr_api_mac_info->dhcp_use;
...
int wifi_mgmr_api_ip_got(uint32_t ip, uint32_t mask, uint32_t gw, uint32_t dns1, uint32_t dns2, long long ip_lease_time)
{
    wifi_mgmr_msg_t *msg;
    ...
    ipgot->dns1 = dns1;
    ipgot->dns2 = dns2;
    ipgot->ip_lease_time = ip_lease_time;
    wifi_mgmr_event_notify(msg);
