...
int bl_send_add_if(struct bl_hw *bl_hw, const unsigned char *mac, const uint8_t *bssid,
                     enum nl80211_iftype iftype, bool p2p, struct mm_add_if_cfm *cfm)
{
    ...
    if (bssid && !MAC_ADDR_CMP(bssid, mac_addr_bcst.array) && !MAC_ADDR_CMP(bssid, mac_addr_zero.array)) {
        memcpy(&(add_if_req_param->bssid.array[0]), bssid, ETH_ALEN);
    } else {
        memcpy(&(add_if_req_param->bssid.array[0]), &mac_addr_bcst, ETH_ALEN);
    }
    printf("[WF] add if bssid = %02X:%02X:%02X:%02X:%02X:%02X\r\n",
        add_if_req_param->bssid.array[0],
        add_if_req_param->bssid.array[1],
        add_if_req_param->bssid.array[2],
        add_if_req_param->bssid.array[3],
        add_if_req_param->bssid.array[4],
        add_if_req_param->bssid.array[5]
    );
