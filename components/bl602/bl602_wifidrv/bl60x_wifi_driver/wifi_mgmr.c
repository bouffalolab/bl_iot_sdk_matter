...
#define WIFI_NETIF_ARP_CHECK_TIMEOUT_MSECS   (200)
...
static bool stateGlobalGuard_AP(void *ev, struct event *event )
{
    ...
    if (bl_main_if_add(0, &(wifiMgmr.wlan_ap.netif), &(wifiMgmr.wlan_ap.vif_index), NULL)) {
...
static bool stateIdleGuard_connect(void *ev, struct event *event )
{
    ...
    printf("[WF] add if bssid= %02X:%02X:%02X:%02X:%02X:%02X\r\n",
        profile_msg->mac[0],
        profile_msg->mac[1],
        profile_msg->mac[2],
        profile_msg->mac[3],
    ...
    if (bl_main_if_add(1, &wifiMgmr.wlan_sta.netif, &wifiMgmr.wlan_sta.vif_index, (const uint8_t *)profile_msg->mac)) {
      ...
      void *newStateData )
{
    ...
    index = wifi_mgmr_valid_profile_get(&wifiMgmr);
    if (-1 == index){
        printf("No valid profile used\r\n");
        return;
    }
    profile = &(wifiMgmr.profiles[index]);

#if 0
    if (ipgot->ip && !MAC_ADDR_CMP(wifiMgmr.wlan_sta.mac, &mac_addr_zero) && !MAC_ADDR_CMP(wifiMgmr.wlan_sta.mac, &mac_addr_bcst)) {
        printf("add arp table ip: 0x%lx, mac: %02x:%02x:%02x:%02x:%02x:%02x\r\n",
            ipgot->ip,
            wifiMgmr.wlan_sta.mac[0],
            wifiMgmr.wlan_sta.mac[1],
            wifiMgmr.wlan_sta.mac[2],
            wifiMgmr.wlan_sta.mac[3],
            wifiMgmr.wlan_sta.mac[4],
            wifiMgmr.wlan_sta.mac[5]
        );
        wifi_netif_arp_add(&(wifiMgmr.wlan_sta.netif), ipgot->ip, wifiMgmr.wlan_sta.mac);
    }

    if (ipgot->gw){
        printf("start add or request arp for gw ipgot->gw: 0x%lx, profile->gw: 0x%lx, gw mac: %02x:%02x:%02x:%02x:%02x:%02x\r\n",
            ipgot->gw,
            profile->net_conf.gw,
            profile->net_conf.gw_mac[0],
            profile->net_conf.gw_mac[1],
            profile->net_conf.gw_mac[2],
            profile->net_conf.gw_mac[3],
            profile->net_conf.gw_mac[4],
            profile->net_conf.gw_mac[5]
        );
        if (ipgot->gw == profile->net_conf.gw && !MAC_ADDR_CMP(profile->net_conf.gw_mac, &mac_addr_zero) && !MAC_ADDR_CMP(profile->net_conf.gw_mac, &mac_addr_bcst)){
            printf("start wifi_netif_arp_add\r\n");
            memcpy(wifiMgmr.wlan_sta.netif.arp_gw_ethaddr, profile->net_conf.gw_mac, 6);
            wifi_netif_arp_add(&(wifiMgmr.wlan_sta.netif), profile->net_conf.gw, profile->net_conf.gw_mac);
        } else{
            printf("start wifi_netif_arp_request\r\n");
            wifi_netif_arp_request(&(wifiMgmr.wlan_sta.netif), ipgot->ip, ipgot->gw);
        }
    }
#endif

    profile->net_conf.ip = ipgot->ip;
    profile->net_conf.mask = ipgot->mask;
    profile->net_conf.gw = ipgot->gw;
    profile->net_conf.dns1 = ipgot->dns1;
    profile->net_conf.dns2 = ipgot->dns2;
    if (ipgot->ip_lease_time > 0) {
        profile->ip_lease_time = ipgot->ip_lease_time;
    }
}
...
#else
    taskENTER_CRITICAL();
    wifi_mgmr_profile_get(&wifiMgmr, &profile_msg);
    ip = profile_msg.net_conf.ip;
    mask = profile_msg.net_conf.mask;
    gw = profile_msg.net_conf.gw;
    dns1 = profile_msg.net_conf.dns1;
    dns2 = profile_msg.net_conf.dns2;
    taskEXIT_CRITICAL();
#endif
...
static void stateConnectedIPNoEnter(void *stateData, struct event *event )
{
    ...
    wifi_mgmr_profile_msg_t profile_msg;
    wifi_netif_ip_info_t ip_info;
    //long long time_now, time_out;

    stateConnectedIPNo_data = stateData;
    os_printf(DEBUG_HEADER "Entering %s state\r\n", stateConnectedIPNo_data->name);
//#ifndef GLOBAL_NETBUS_WIFI_APP_ENABLE
#if 0
    ...
    memset(&ip_info, 0, sizeof(wifi_netif_ip_info_t));
    wifi_mgmr_profile_get(&wifiMgmr, &profile_msg);
    os_printf(DEBUG_HEADER "try to get IP...\r\n");
    printf("Wifi MGMR Default IP:%u.%u.%u.%u, "
        "MASK: %u.%u.%u.%u, "
        "Gateway: %u.%u.%u.%u, "
        "dns1: %u.%u.%u.%u, "
        "dns2: %u.%u.%u.%u\r\n",
        (unsigned int)((profile_msg.net_conf.ip & 0x000000FF) >> 0),
        (unsigned int)((profile_msg.net_conf.ip & 0x0000FF00) >> 8),
        (unsigned int)((profile_msg.net_conf.ip & 0x00FF0000) >> 16),
        (unsigned int)((profile_msg.net_conf.ip & 0xFF000000) >> 24),
        (unsigned int)((profile_msg.net_conf.mask & 0x000000FF) >> 0),
        (unsigned int)((profile_msg.net_conf.mask & 0x0000FF00) >> 8),
        (unsigned int)((profile_msg.net_conf.mask & 0x00FF0000) >> 16),
        (unsigned int)((profile_msg.net_conf.mask & 0xFF000000) >> 24),
        (unsigned int)((profile_msg.net_conf.gw & 0x000000FF) >> 0),
        (unsigned int)((profile_msg.net_conf.gw & 0x0000FF00) >> 8),
        (unsigned int)((profile_msg.net_conf.gw & 0x00FF0000) >> 16),
        (unsigned int)((profile_msg.net_conf.gw & 0xFF000000) >> 24),
        (unsigned int)((profile_msg.net_conf.dns1 & 0x000000FF) >> 0),
        (unsigned int)((profile_msg.net_conf.dns1 & 0x0000FF00) >> 8),
        (unsigned int)((profile_msg.net_conf.dns1 & 0x00FF0000) >> 16),
        (unsigned int)((profile_msg.net_conf.dns1 & 0xFF000000) >> 24),
        (unsigned int)((profile_msg.net_conf.dns2 & 0x000000FF) >> 0),
        (unsigned int)((profile_msg.net_conf.dns2 & 0x0000FF00) >> 8),
        (unsigned int)((profile_msg.net_conf.dns2 & 0x00FF0000) >> 16),
        (unsigned int)((profile_msg.net_conf.dns2 & 0xFF000000) >> 24)
    );
#if 0
    ip_info.ip = profile_msg.net_conf.ip;
    ip_info.mask = profile_msg.net_conf.mask;
    ip_info.gw = profile_msg.net_conf.gw;
    ip_info.dns1 = profile_msg.net_conf.dns1;
    ip_info.dns2 = profile_msg.net_conf.dns2;
    ip_info.dhcp_use = profile_msg.dhcp_use;
    memcpy(ip_info.gw_mac, profile_msg.net_conf.gw_mac, 6);
    if (1 == profile_msg.dhcp_use){
        if (!ip_info.ip || !ip_info.mask || !ip_info.gw){
            os_printf(DEBUG_HEADER "DHCP Starting...\r\n");
            wifi_netif_dhcp_start(&(wifiMgmr.wlan_sta.netif));
        } else {
            os_printf(DEBUG_HEADER "use dynamic IP\r\n");
            bl60x_current_time_us(&time_now);
            time_out = time_now + WIFI_NETIF_ARP_CHECK_TIMEOUT_MSECS * 1000;
            if (time_out < profile_msg.ip_lease_time){
                os_printf("ip lease time is not end, no need arp check\r\n");
                wifi_netif_network_ip_set(&(wifiMgmr.wlan_sta.netif), &ip_info, 0);
            } else {
                os_printf("ip lease time ended, use arp to check ip\r\n");
                wifi_netif_dhcp_start(&(wifiMgmr.wlan_sta.netif));
                wifi_netif_network_ip_set(&(wifiMgmr.wlan_sta.netif), &ip_info, WIFI_NETIF_ARP_CHECK_TIMEOUT_MSECS);
            }
        }
    } else if (0 == profile_msg.dhcp_use){
        if (!ip_info.ip || !ip_info.mask || !ip_info.gw){
            os_printf(DEBUG_HEADER "Invalid IP! restart DHCP to get IP\r\n");
            wifi_netif_dhcp_start(&(wifiMgmr.wlan_sta.netif));
        } else {
            os_printf(DEBUG_HEADER "use static IP\r\n");
            wifi_netif_network_ip_set(&(wifiMgmr.wlan_sta.netif), &ip_info, 0);
        }
    }
#else
...
static void disconnect_retry(timer_cb_arg_t data)
{
    ...
    if (wifi_mgmr_profile_get(&wifiMgmr, &(stateData->profile_msg))) {
        ...
        wifi_mgmr_profile_update(&wifiMgmr, 1);
...
int wifi_mgmr_state_get_internal2(int *state, int *state2)
{
    const struct state *m_state;
    int s_code = 0;
    ...
    if (m_state == &stateIdle) {
        if (wifiMgmr.inf_ap_enabled) {
            ...
            *state = WIFI_STATE_IDLE;
            if (s_code == WLAN_FW_4WAY_HANDSHAKE_ERROR_PSK_TIMEOUT_FAILURE){
                *state2 = WIFI_STATE_PSK_ERROR;
            } else if (s_code == WLAN_FW_SCAN_NO_BSSID_AND_CHANNEL){
                *state2 = WIFI_STATE_NO_AP_FOUND;
            }
        }
    } else if (m_state == &stateConnecting) {
        if (wifiMgmr.inf_ap_enabled) {
...
void wifi_mgmr_set_profile_timestamp(long long timestamp)
{
    wifi_mgmr_profile_t *profile;
    int i;
    ...
    for (i = 0; i < sizeof(wifiMgmr.profiles)/sizeof(wifiMgmr.profiles[0]); i++) {
        if (1 == wifiMgmr.profiles[i].isUsed) {
            profile = &(wifiMgmr.profiles[i]);
            break;
        }
    }
    if (NULL == profile){
